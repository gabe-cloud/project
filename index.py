...
class Registration:
    ...
    def __init__(self, window):
        ...
        # Button Add new file
        ttk.Button(frame, text="Save", command=self.add_file).grid(
        ...
        # Buttons
        ttk.Button(text="BORRAR", command=self.delete_file).grid(
            row=7, column=0, sticky=W + E
        )
        ttk.Button(text="EDITAR", command=self.edit_file).grid(
            row=7, column=2, sticky=W + E
        )
    ...
    def validation(self):
        return (
            len(self.id.get()) != 0
            and len(self.title.get()) != 0
            and len(self.description.get()) != 0
        )
    ...
    def edit_file(self):
        ...
        Entry(
            self.edit_wind,
            textvariable=StringVar(self.edit_wind, value=id),
            state="readonly",
        ).grid(row=0, column=2)
        # New id
        ...
        Entry(
            self.edit_wind,
            textvariable=StringVar(self.edit_wind, value=old_title),
            state="readonly",
        ).grid(row=2, column=2)
        # New title
        ...
        Entry(
            self.edit_wind,
            textvariable=StringVar(self.edit_wind, value=old_description),
            state="readonly",
        ).grid(row=4, column=2)
        # New description
        ...
        Button(
            ...
            command=lambda: self.edit_records(
                new_id.get(), id, new_title.get(), new_description.get()
            ),
...
if __name__ == "__main__":
    ...
    window.iconbitmap(
        "C:\\Users\\gjrzr\\Desktop\\TRY\\uneg-logo-2D2635F1F5-seeklogo.com.ico"
    )
    window.mainloop()
